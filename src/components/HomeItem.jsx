...
const HomeItem = ({item})=> {
    ...
    const isItemInWishlist = wishlistItems.some(wishlistItem => wishlistItem._id === item._id) ;
    ...
    const deleteItem = () => {
        ...
        .then((data) => {
            ...
            // Optionally, you can also remove the item from the Redux store
    ...
    <div class="item-container" >
        {isItemInWishlist ?  <FiHeart className="wishlist-icon" color="red" size={32} onClick={handleAddToWishlist}/> : <FaRegHeart className="wishlist-icon" size={32} onClick={handleAddToWishlist}/>}
      ...
      {elementFound ? <button type="button" class="btn-add-bag btn btn-danger" onClick={handleRemoveFromBag}><MdDeleteForever />Remove from bag</button> : <button type="button" class="btn-add-bag btn btn-success" onClick={handleAddtoBag}>{<MdAddShoppingCart />}Add to Bag</button>}
      {atMyPostsPage && <button type="button" onClick={deleteItem} class="btn-delete-item btn btn-danger">Delete Item</button>}
